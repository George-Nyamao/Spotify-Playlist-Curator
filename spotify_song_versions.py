...
def find_relevant_song_versions(track_name, artist_name):
    """
    ...
    for track in tracks:
        ...
        # FILTER 1: Check if Leon Thomas is the MAIN artist
        if main_artist.lower() != artist_name.lower():
            continue
            
        # FILTER 2: Check if the track name starts with or closely matches "MUTT"
        track_name_lower = track['name'].lower()
        search_name_lower = track_name.lower()
        
        # Accept tracks that start with "MUTT" or contain "MUTT" as a distinct word
...
# Example usage
versions = find_relevant_song_versions("MUTT", "Leon Thomas")

print(f"Found {len(versions)} relevant versions of MUTT by Leon Thomas:")
print("=" * 80)
